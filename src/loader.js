...
//     bot,
//     targetPoint,
//     { jump, sprint, jumpAfter = 0 },
//     maxTicks = 20
//   ) {
//     const reached = (state) => {
//       if (!state) return false;
//       return isPlayerOnBlock(state.pos, targetPoint, true);
//     };

//     const controller = getController(targetPoint, jump, sprint, jumpAfter);
//     const resultState = simulateUntil(bot, reached, controller, maxTicks);

//     if (!resultState || resultState.isInLava) return null;
//     return resultState;
//   }

//   function canSprintJump(bot, targetPoint) {
//     if (canWalkJump(bot, targetPoint)) return false;

//     const sprintJump = canReachWith(bot, targetPoint, {
//       jump: true,
//       sprint: true,
//     });
//     const walkJump = canReachWith(bot, targetPoint, {
//       jump: false,
//       sprint: true,
//     });
...
//     const walkJumpValid =
//       walkJump &&
//       isPlayerOnBlock(walkJump.pos, targetPoint, true) &&
//       walkJump.onGround &&
//       walkJump.pos.y - bot.entity.position.y > 0;

//     if (!sprintJump || walkJumpValid) return false;

//     const fromPos = bot.entity.position;
//     const jumpDistXZ =
//       Math.abs(sprintJump.pos.x - fromPos.x) +
//       Math.abs(sprintJump.pos.z - fromPos.z);
//     const deltaXZ =
//       Math.abs(sprintJump.pos.x - targetPoint.x) +
//       Math.abs(sprintJump.pos.z - targetPoint.z);

//     return jumpDistXZ >= 2 && jumpDistXZ <= 4 && deltaXZ <= 0.5;
//   }

//   function canWalkJump(bot, targetPoint) {
//     const state = canReachWith(bot, targetPoint, { jump: true, sprint: false });
//     if (!state || !state.onGround) return false;

//     const dx = state.pos.x - targetPoint.x;
//     const dz = state.pos.z - targetPoint.z;
//     const distXZ = Math.sqrt(dx * dx + dz * dz);
//     const deltaY = state.pos.y - bot.entity.position.y;

//     // Accept both cardinal and diagonal jump-ups
//     return distXZ <= 1.45 && deltaY >= 1;
//   }
...
//     //this cell is a ladder we should look directly up and hold jump
//     if (cell.attributes.ladder) {
//       if (bot.ashfinder.debug) showPathParticleEffect(point);

//       const ladderBlockPos = point.floored();
//       const ladderBlock = bot.blockAt(ladderBlockPos);

//       // Check if bot is inside ladder block
//       if (
//         (ladderBlock.name !== "ladder" || !ladderBlock.name !== "vines") &&
//         !bot.entity.isCollidedHorizontally
//       ) {
//         // Move into ladder block
//         await updateBotLookDirection(botPos, point);
//         bot.setControlState("forward", true);
//         bot.setControlState("jump", false);
//         return false; // wait until bot enters ladder block
//       }

//       // console.log("[Debug] Bot is inside ladder block:", ladderBlockPos);

//       // Now bot is inside ladder block and can climb

//       if (!headLocked) {
//         // Lock look direction to straight up
//         bot.look(0, -90);
//         headLocked = true;
//       }
...
//       // Instead of instantly resolving cell when close,
//       // wait until the bot's Y is *above* the ladder block by a bit (e.g. 1 block),
//       // so that the bot fully climbed this ladder block

//       // The ladder block's Y is ladderBlockPos.y
//       // The target cell Y is point.y (should be same or similar)
//       // We want bot's Y to be at least ladderBlockPos.y + 1 to resolve

//       if (botPos.y > ladderBlockPos.y + 0.9) {
//         // Finished climbing this ladder block
...
//         resetMovementState(shouldSlowDown, true);
//         straightPathOptions = null; // Resolve this ladder cell

//         return true;
//       }

//       return false; // keep climbing this ladder block
//     }
...
//     await handleClimbingAndJumping(point, botPos, cell.attributes);
...
//                 await placeBlockAtTarget(poss, poss.dir, blockPlace);
//                 placing = false;
//               }
//             } catch (error) {
...
//         placing = true;
//         await equipBlockIfNeeded(blockPlace);
//         await placeBlockAtTarget(target, target.dir, blockPlace);
//         await sleep(10);
//       }
//     }
...
//    */
//   async function placeBlockAtTarget(cell, dir, blockPlace) {
//     const pos = cell;
//     const block = bot.blockAt(pos);
...
//   async function resetMovementState(shouldSlowDown, onLadder = false) {
//     // bot.setControlState("sprint", false);
//     // await sleep(1)
//     if (!onLadder) bot.setControlState("jump", false);
//     // bot.setControlState("forward", false);
//     if (straightPathOptions) straightPathOptions.resolve();
//     if (!onLadder) headLocked = false;
...
//    * @returns
//    */
//   async function handleClimbingAndJumping(point, botPos, attributes) {
//     if (digging) return;

//     //
//     let shouldWalkJump = attributes.nJump
//       ? true
//       : false && canWalkJump(bot, point);

//     //&&
//     let shouldSprintJump = attributes.sJump
//       ? true
//       : false && canSprintJump(bot, point);

//     if (attributes.swimming && bot.entity.isInWater) {
//       handleWaterMovement(botPos, attributes);
...
//     const yDist = bot.entity.position.y - ladderBlock.position.y;
//     if (yDist > 0) {
//       headLocked = true;
//       climbing = true;
//       bot.clearControlStates();
...
//   function handleWaterMovement(botPos, attributes) {
...
//     if (bot.ashfinder.debug) console.log("Iterations", result.iterations);
...
//       const promisingNode = result.bestNode;

//       if (promisingNode) {
//         const targetPathFromPromising = await astar(
//           promisingNode.worldPos,
//           position, // original goal
//           bot,
//           createEndFunc(goal),
//           bot.ashfinder.config
//         );
...
//             promisingNode.worldPos,
//             bot,
//             (pos) => pos.equals(promisingNode.worldPos),
//             bot.ashfinder.config
//           );
...
//             complexPathPoints = fullPath;
//             bot.ashfinder.path = fullPath;
//             extractPathPoints();
//             return await walkPathLoop(fullPath, pathNumber); // Make this function for clean looping
//           }
//         }
//       }
...
//       return await path(goal, options); // fallback to normal retry
//     }
...
//   async function walkPathLoop(pathPoints, pathNumber) {
//     let slowDown = false;
//     while (pathPoints.length > 0) {
//       if (pathPoints.length <= 1) slowDown = true;
//       const movement = pathPoints[0];
...
//       if (currentCalculatedPathNumber > pathNumber || pathPoints === null)
//         return;
...
//    *
//    * @param {Goal} goal - The goal to reach, which contains the target position.
//    * @return {object} An object containing the generated path and its cost.
//    */
//   bot.ashfinder.generatePath = async (goal) => {
//     const endFunc = createEndFunc(goal);
